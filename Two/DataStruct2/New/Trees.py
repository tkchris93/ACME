...
class BST(object):
    ...
    def remove(self, data):
        ...
        def _find_SS(node): #find and return the SS (smallest successor) node
        ...
        def _remove_two(target_node, value): # remove node with two children
            SS = _find_SS(target_node)
            target_node.data = SS.data
            if SS.right is not None:
                _remove_one(SS.prev, SS.data)
            else:
                _remove_leaf(SS.prev, SS.data)
